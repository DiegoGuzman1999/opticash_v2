# Script final para probar el Dockerfile.docs
# Uso: .\scripts\test-dockerfile-final.ps1
...
try {
    Write-Host "Ejecutando: docker build -f Dockerfile.docs -t opticash-docs:test ." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "Ejecutando: docker run -d --name opticash-docs-test -p 8080:80 opticash-docs:test" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "No se pudieron obtener los logs del contenedor" -ForegroundColor Yellow
}
